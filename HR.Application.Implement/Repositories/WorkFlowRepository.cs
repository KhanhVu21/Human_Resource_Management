...
public class WorkFlowRepository : IWorkFlowRepository
{
    ...
    private static async Task<TemplateApi<WorkflowTemplateDto>> MoveOnItemInWorkFlow(Guid idWorkFlowInstance,
        ...
        string fullName, WorkflowInstances workflowInstances, List<WorkflowStep> workflowStep, IDbConnection connection,
        IDbTransaction tran, User user)
    {
        ...
        workflowInstances.UnitId = workflowStep.ElementAt(workflowInstances.CurrentStep - 1).IdUnitAssign;
    ...
    private static async Task<TemplateApi<WorkflowTemplateDto>> ApproveItemInWorkFlow(Guid idWorkFlowInstance,
        string message, Guid idUserCurrent,
        string fullName, IDbConnection connection, IDbTransaction tran, List<WorkflowStep> workflowStep,
        WorkflowInstances workflowInstances, User user)
    {
        ...
        //getging outCome field in workflowStep table
        ...
        if (workflowTemplate.WorkflowCode == AppSettings.TicketLaborEquipment)
        {
            ...
            switch (ticketLaborEquipments.Type)
            {
                    ...
                    foreach (var item in ticketLaborEquipmentDetails)
                    {
                        for (var i = 0; i < item.Quantity; i++)
                        {
                            laborEquipmentUnits.Add(new LaborEquipmentUnit()
                            {
                                ...
                                CreatedDate = DateTime.Now,
                                Status = 0,
                                EquipmentCode = DateTimeOffset.Now.ToUnixTimeMilliseconds() + i.ToString(),
                ...
                case 2:
                    ...
                    break;
                case 3:
                    // //updating all of status = 0 in LaborEquipmentUnit table by list id
                    // await connection.ExecuteAsync(
                    //     LaborEquipmentUnitSqlQueries.QueryUpdateStatusToUsingLaborEquipmentUnitByIds,
                    //     new
                    //     {
                    //         Status = 0, IdEmployee = , IdUnit = ticketLaborEquipments.IdUnit,
                    //         Id = laborEquipmentUnit.Select(e => e.Id)
                    //     },
                    //     tran);
    ...
    /// <summary>
    /// using this function to request to change infoemation item in work flow
    /// this function is using the params like a above function
    /// </summary>
    /// <param name="idWorkFlowInstance"></param>
    /// <param name="message"></param>
    ...
    private static async Task<TemplateApi<WorkflowTemplateDto>> RequestToChangeItemInWorkFlow(Guid idWorkFlowInstance,
        ...
        IDbConnection connection, IDbTransaction tran, WorkflowInstances workflowInstances,
        List<WorkflowStep> workflowStep, User user)
    {
        Diary diary;
        ...
        Guid idUserCurrent, Guid idTemplate)
    {
        ...
        var roles = (await connection.QueryAsync<Role>(RoleSqlQueries.GetRoleByIdUser, new { IdUser = idUserCurrent }))
            .ToList();
        //get data by id in WorkflowHistory table
        var workflowHistories = (await connection.QueryAsync<WorkflowHistory>(
            WorkflowHistorySqlQueries.QueryGetAllWorkflowHistories)).ToList();
        ...
        foreach (var item in data)
        {
            ...
            {
                workflowHistories = workflowHistories.Where(e => e.IdWorkFlowInstance == itemWorkflowInstances.Id)
                    .ToList();
            }
            else
            {
                workflowHistories = workflowHistories.Where(e =>
    ...
    /// <typeparam name="T"></typeparam>
    /// <returns></returns>
    private List<T> AssignValueToIListItem<T>(List<T> data, List<WorkflowInstances> workflowInstances,
        List<WorkflowStep> workflowSteps, bool isAdminOrUserRequest, User user, List<Role> roles)
    {
    ...
    /// <typeparam name="T"></typeparam>
    /// <returns></returns>
    private T? SetValueToItem<T>(T item, List<WorkflowStep> workflowSteps, List<WorkflowInstances> workflowInstances,
        bool isAdminOrUserRequest, User user, List<Role> roles)
    {
    ...
    /// <typeparam name="T"></typeparam>
    /// <returns></returns>
    private Guid GetIdValue<T>(T item)
    {
        var objectType = item!.GetType();
